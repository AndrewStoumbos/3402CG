...
extern code gen_code_var_decls(var_decls_t vds);

// Generate code for a single <var-decl>, vd,
// There are 2 instructions generated for each identifier declared
// (one to allocate space and another to initialize that space)
extern code gen_code_var_decl(var_decl_t vd);

// Generate code for the identififers in idents with type t
// in reverse order (so the first declared are allocated last).
// There are 2 instructions generated for each identifier declared
// (one to allocate space and another to initialize that space)
extern code gen_code_idents(ident_list_t idents,
				expr_kind_e t);

// Generate code for stmt
extern code gen_code_stmt(stmt_t stmt);

// Generate code for stmt
extern code gen_code_assign_stmt(assign_stmt_t stmt);

// Generate code for stmt
extern code gen_code_begin_stmt(call_stmt_t stmt);

// Generate code for the list of statments given by stmts to out
extern code gen_code_stmts(stmts_t stmts);

// Generate code for the if-statment given by stmt
extern code gen_code_if_stmt(if_stmt_t stmt);

// Generate code for the read statment given by stmt
extern code gen_code_read_stmt(read_stmt_t stmt);

// Generate code for the write statment given by stmt.
extern code gen_code_write_stmt(print_stmt_t stmt);
...
extern code gen_code_expr(expr_t exp);

// Generate code for the expression exp
// putting the result on top of the stack,
// and using V0 and AT as temporary registers
// May also modify SP, HI,LO when executed
extern code gen_code_binary_op_expr(binary_op_expr_t exp);
...
// and using V0 and AT as temporary registers
// May also modify SP, HI,LO when executed
extern code gen_code_op(token_t op, expr_kind_e typ);
...
extern code gen_code_arith_op(token_t arith_op);
...
// and using V0 and AT as temporary registers
// Also modifies SP when executed
extern code gen_code_rel_op(token_t rel_op, expr_kind_e typ);

// Generate code to put the value of the given identifier
// on top of the stack
// Modifies T9, V0, and SP when executed
extern code gen_code_ident(ident_t id);

// Generate code to put the given number on top of the stack
// Modifies V0 when executed
extern code gen_code_number(number_t num);

// Generate code for the expression exp
// putting the result on top of the stack,
// and using V0 and AT as temporary registers
// May also modify SP, HI,LO when executed
extern code gen_code_logical_not_expr(expr_t exp);
